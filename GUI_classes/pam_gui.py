...
class PAM_class(StartingGui):
    def __init__(self):
        super(PAM_class, self).__init__(name="PAM", twinx=False, first_plot=True, second_plot=False,
                                        function=self.start_PAM, extract=False, stretch_plot=False)
    ...
    def start_PAM(self):
        ...
        PAM = KMedoids_gui(n_cluster=self.n_medoids, log=self.log, ax=self.ax1, canvas=self.canvas_up,
                           save_fig=self.save_plots, ind_run=self.ind_run, delay=self.delay)
...
class KMedoids_gui:
    def __init__(self, n_cluster, log, ax, canvas, save_fig, ind_run, delay,
                 max_iter=10, tol=0.001, start_prob=0.8, end_prob=0.99, random_state=42):
    ...
    def __start_algo(self):
        ...
        self.log.appendPlainText("clusters_distances: {}".format(self.cluster_distances))
    ...
    def __swap_and_recalculate_clusters(self):
        ...
        for medoid in self.medoids:  # for each medoid
            ...
            if is_shortest_medoid_found is False:
                self.log.appendPlainText("no better medoids found, keep: {}".format(medoid))
    ...
    def plot_pam_gui(self, data, ax, canvas, cl, ind_run, ind_fig, name="PAM", save_plots=False):
        ...
        for i, el in enumerate(list(cl.values())):
            ax.scatter(np.array(data)[el, 0], np.array(data)[el, 1], s=300, color=colors[i % 18], edgecolor="black")

        # plot centers of mass, marked with an X
        for i, el in enumerate(list(cl.keys())):
            ax.scatter(np.array(data)[el, 0], np.array(data)[el, 1], s=500, color="red", marker="X", edgecolor="black")
