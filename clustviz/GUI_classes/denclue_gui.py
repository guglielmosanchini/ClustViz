...
class DENCLUE_class(StartingGui):
    def __init__(self):
        ...
        self.checkbox_contour.stateChanged.connect(
            lambda: self.number_of_plots_gui(1)
        )
        self.checkbox_3dplot.stateChanged.connect(
            lambda: self.number_of_plots_gui(2)
        )
        self.checkbox_clusters.stateChanged.connect(
            lambda: self.number_of_plots_gui(3)
        )

        # influence plot
        self.button_infl_denclue.clicked.connect(
            lambda: self.plot_infl_gui(
                ax=self.ax_infl, canvas=self.canvas_infl
            )
        )
    ...
    def DENCLUE_gui(
        self, data, s, xi, xi_c, tol, prec, save_plots, dist="euclidean"
    ):
        ...
        self.log.appendPlainText(
            "Number of populated cubes: {}".format(len(z))
        )
    ...
    def plot_3d_both_gui(
        ...
        for i, a in enumerate(range(prec * 10)):
            ...
            for j, b in enumerate(range(prec * 10)):
                ...
                z[i, j] = gauss_dens(
                    x=np.array([xx[i][a], yy[i][b]]), D=data, s=s
                )
                if xi is not None:
                    if z[i, j] >= xi:
                        z_xi[i, j] = z[i, j]
    ...
    def plot_clust_dict_gui(
        self, data, lab_dict, ax, canvas, save_plots, ind_fig
    ):
    ...
    def openFinalStepWindow_4(self, canvas):
        self.w4 = FinalStepWindow(
            canvas=canvas, window_title="Influence function"
        )
        self.w4.show()
