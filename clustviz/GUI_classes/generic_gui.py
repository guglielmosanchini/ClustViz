...
class StartingGui(QWidget):
    def __init__(
        ...
        fold_dir = "./Images/"
        folders = [
            el for el in os.listdir(fold_dir) if el.startswith(self.name + "_")
        ]
        folders.sort()
        if len(folders) == 0:
            self.ind_run = 0
        ...
        self.slider = LabeledSlider(
            minimum=0, maximum=3, interval=1, single_step=0.5
        )
        ...
        if extract is True:
            ...
            self.button_extract.clicked.connect(
                lambda: self.start_EXTRACT_OPTICS()
            )
    ...
    def label_initialization(self):
        ...
        if (self.name == "OPTICS") or (self.name == "DBSCAN"):
            ...
            if self.name == "CLARANS":
                ...
                self.line_edit_numlocal_clarans.setText(
                    str(self.numlocal_clarans)
                )
                ...
                self.maxneighbors_clarans_validator = QIntValidator(
                    1, 1000, self
                )
            ...
            self.line_edit_init_clust_cham.setValidator(
                self.init_clust_cham_validator
            )

            # alpha_cham LABEL
            self.label_alpha_cham = QLabel(self)
            self.label_alpha_cham.setText("alpha:")
            self.label_alpha_cham.setToolTip(
                "The exponent of relative closeness."
            )
            ...
            self.line_edit_sigma_denclue.setValidator(
                self.sigma_denclue_validator
            )
            ...
            self.line_edit_xi_c_denclue.setValidator(
                self.xi_c_denclue_validator
            )
            ...
            self.line_edit_prec_denclue.setValidator(
                self.prec_denclue_validator
            )
    ...
    def buttons_groupbox_initialization(self):
        ...
        if (self.name == "OPTICS") or (self.name == "DBSCAN"):
            ...
            self.gridlayout_but.addWidget(
                self.line_edit_alpha_cure, 4, 2, 1, 2
            )
            if self.name == "LARGE CURE":
                ...
                self.gridlayout_but.addWidget(
                    self.checkbox_saveimg, 8, 0, 1, 2
                )
                self.gridlayout_but.addWidget(self.checkbox_gif, 8, 2, 1, 2)
                self.gridlayout_but.addWidget(
                    self.button_delete_pics, 9, 0, 1, 4
                )
            elif self.name == "CURE":
                ...
                self.gridlayout_but.addWidget(
                    self.checkbox_saveimg, 7, 0, 1, 2
                )
                self.gridlayout_but.addWidget(self.checkbox_gif, 7, 2, 1, 2)
                self.gridlayout_but.addWidget(
                    self.button_delete_pics, 8, 0, 1, 4
                )
            ...
            self.gridlayout_but.addWidget(
                self.line_edit_numlocal_clarans, 3, 1
            )
            self.gridlayout_but.addWidget(
                self.label_maxneighbors_clarans, 4, 0
            )
            self.gridlayout_but.addWidget(
                self.line_edit_maxneighbors_clarans, 4, 1
            )
            ...
            if self.name == "CHAMELEON":
                ...
                self.gridlayout_but.addWidget(self.checkbox_gif, 8, 1)
                self.gridlayout_but.addWidget(
                    self.button_delete_pics, 9, 0, 1, 0
                )
                ...
                self.gridlayout_but.addWidget(
                    self.button_delete_pics, 11, 0, 1, 0
                )
            ...
            self.gridlayout_plot_checkboxes = QGridLayout(
                self.groupbox_plots_denclue
            )
            ...
            self.gridlayout_plot_checkboxes.addWidget(
                self.checkbox_contour, 0, 1
            )
            self.gridlayout_plot_checkboxes.addWidget(
                self.checkbox_3dplot, 1, 0
            )
            self.gridlayout_plot_checkboxes.addWidget(
                self.checkbox_clusters, 1, 1
            )
            self.gridlayout_plot_checkboxes.addWidget(
                self.checkbox_saveimg, 2, 0
            )
            self.gridlayout_plot_checkboxes.addWidget(
                self.button_infl_denclue, 2, 1
            )
    ...
    def n_clust_error(self, medoids=False, init_clust=False):
        ...
        if medoids is False:
            ...
            cond = int(self.line_edit_n_clust.text()) >= int(
                self.line_edit_np.text()
            )
            text = "clusters"

        else:

            cond = int(self.line_edit_n_medoids.text()) >= int(
                self.line_edit_np.text()
            )
            text = "medoids"
    ...
    def verify_input_parameters(self, extract=False):
        ...
        self.param_check = True
        check_n_points = self.n_points_validator.validate(
            self.line_edit_np.text(), 0
        )
        ...
        if (self.name == "OPTICS") or (self.name == "DBSCAN"):
            ...
            if extract is False:
                check_eps = self.eps_validator.validate(
                    self.line_edit_eps.text(), 0
                )
                check_mp = self.mp_validator.validate(
                    self.line_edit_mp.text(), 0
                )
            ...
            if self.name == "LARGE CURE":
                ...
                self.show_error_message(
                    check_p_cure,
                    "The parameter p can be an {0}, {1} or {2}.".format(
                        2, 3, 4
                    ),
                )
                self.show_error_message(
        ...
        for i in range(4):
            ...
            if i == 2:
                self.axes_list.append(
                    self.canvas_list[i].figure.add_subplot(
                        111, projection="3d"
                    )
                )
            else:
        ...
        if np.sum(pic_list) == 1:
            ...
            if 2 in pic_indexes:
                self.gridlayout_plots.addWidget(
                    canvas_list[pic_indexes[0]], 0, 0
                )
                self.gridlayout_plots.addWidget(
                    canvas_list[pic_indexes[1]], 0, 1
                )
            else:
                self.gridlayout_plots.addWidget(
                    canvas_list[pic_indexes[0]], 0, 0
...
class GraphWindow(QMainWindow):
    def __init__(self, example_index):
        ...
        pixmap = QPixmap(
            "./Images/ClaransGraphExamples/graph_{:02}.png".format(
                example_index
            )
        )
