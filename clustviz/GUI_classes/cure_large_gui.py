...
class LARGE_CURE_class(StartingGui):
    ...
    def start_LARGE_CURE(self):
        ...
        self.SetWindows(
            number=self.p_cure, first_run_boolean=self.first_run_occurred_mod
        )
    ...
    def point_plot_mod2_gui(
        ...
        ax.scatter(
            data[:, 0], data[:, 1], s=300, color="lime", edgecolor="black"
        )
        ...
        if save_plots is True:
            ...
            if ind_fig_bis is not None:
                canvas.figure.savefig(
                    "./Images/{}_{:02}/fig_{:02}_{:02}.png".format(
        ...
        if last_reps is not None:
            ...
            for ind, i in enumerate(range(0, len(a))):
                point = (
                    a.iloc[i].name.replace("(", "").replace(")", "").split("-")
                )
            ...
            flat_reps = [
                item
                for sublist in list(last_reps.values())
                for item in sublist
            ]
    ...
    def cure_gui(
        ...
        if preprocessed_data is None:
            ...
            if partial_index is not None:
                ...
                a = pd.DataFrame(
                    index=[str(i) for i in range(len(data))], columns=col
                )
            ...
            if partial_index is not None:
                ...
                clusters = {
                    str(i): np.array(data[i]) for i in range(len(data))
                }
            ...
            if partial_index is not None:
                rep = {
                    partial_index[i]: [data[int(i)]] for i in range(len(data))
                }
            else:
        ...
        while len(heap) > k:
            ...
            heap = dict(
                OrderedDict(sorted(heap.items(), key=lambda kv: kv[1]))
            )
            ...
            if plotting is True:
                ...
                a.loc["(" + u + ")" + "-" + "(" + u_cl + ")", :] = a.loc[
                    u
                ].fillna(0) + a.loc[u_cl].shift(dim1, fill_value=0)
                a = a.drop(u, 0)
                a = a.drop(u_cl, 0)
    ...
    def cure_sample_part(
        self, u_min=None, f=0.3, d=0.02, n_rep_finalclust=None, delay=0
    ):
        """
        ...
        while True:
            try:
                ...
                n = ceil(
                    self.Chernoff_Bounds_gui(
                        u_min=u_min, f=f, N=len(self.X), k=self.n_clust, d=d
                    )
                )
        ...
        if (self.p_cure is None) and (self.q_cure is None):
            ...
            self.log.appendPlainText(
                "p was automatically set to: {}".format(self.p_cure)
            )
            self.log.appendPlainText(
                "q was automatically set to: {}".format(self.q_cure)
            )
    ...
    def Chernoff_Bounds_gui(self, u_min, f, N, d, k):
        ...
        res = (
            f * N
            + N / u_min * l
            + N / u_min * np.sqrt(l ** 2 + 2 * f * u_min * l)
        )
        self.log.appendPlainText("")
