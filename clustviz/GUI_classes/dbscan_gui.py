...
class DBSCAN_class(StartingGui):
    ...
    def point_plot_mod_gui(
        self, X_dict, point, save_plots=False, ind_fig=None
    ):
        """
        ...
        for i in self.ClustDict:
            self.ax1.scatter(
                X_dict[i][0],
                X_dict[i][1],
                color=colors[self.ClustDict[i] % 12]
                if self.ClustDict[i] != -1
                else colors[-1],
                s=300,
            )
    ...
    def plot_clust_DB_gui(
        self, save_plots=False, circle_class=None, noise_circle=False
    ):
        """
