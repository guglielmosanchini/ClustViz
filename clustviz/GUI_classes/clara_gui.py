...
class ClaraClustering_gui(object):
    ...
    def __init__(
        self, log, ax, canvas, save_fig, ind_run, delay, max_iter=100000
    ):
    ...
    def k_medoids(self, _df, _k, _fn, _niter):
        ...
        self.log.appendPlainText(
            "initial medoids sample: {}".format(medoids_sample)
        )
        ...
        while iter_count < _niter:
            ...
            if best_choices == medoids_sample:
                self.log.appendPlainText(
                    "Best configuration found! best_choices: {}".format(
                        best_choices
                    )
                )
                break
