...
from algorithms.chameleon.chameleon2 import (
    knn_graph_sym,
    prepro_edge,
...
class CHAMELEON2_class(StartingGui):
    ...
    def merge_best2_gui(
        self, graph, df, a, b, m_fact, k, verbose=False, verbose2=True
    ):
    ...
    def flood_fill_gui(self, graph, knn_gr, df):
        ...
        for num in range(max(cl_dict.values()) + 1):
            points = [
                ...
                if list(cl_dict.values())[i] == num
            ]
            restr_dict = {
                list(dic_edge.keys())[i]: dic_edge[i] for i in points
            }
            r_dict = {}
    ...
    def extract_optimal_n_clust_gui(self, h, m, f=1000, eta=2):
        ...
        self.log.appendPlainText("")
        self.log.appendPlainText(
            "Optimal number of clusters: {}".format(opt_n_clust)
        )
