...
def test_dist_mat_gen():
    df_for_dist_mat_gen = pd.DataFrame([[0, 0, 1],
                                        [0, 2, 0]])

    assert dist_mat_gen(df_for_dist_mat_gen).equals(pd.DataFrame([[np.inf, 2], [2, np.inf]]))


def test_update_mat():
    df_for_update_mat = pd.DataFrame([[np.inf, 1, 4], [1, np.inf, 2], [4, 2, np.inf]],
                                     columns=["a", "b", "c"],
                                     index=["a", "b", "c"])
...
def test_dist_mat():
    df_for_dist_mat = pd.DataFrame([[0, 0, 1],
                                    [0, 2, 0]])

    assert dist_mat(df_for_dist_mat, "single").equals(pd.DataFrame([[np.inf, 2], [np.inf, np.inf]]))
