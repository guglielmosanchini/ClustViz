...
from clustviz.chameleon.chameleon2 import cluster2, connected_components, prepro_edge
...
def test_connected_components():
    ...
    pregraph = knn_graph(df, 3, symmetrical=True)
    graph = pre_part_graph(pregraph, 10, df)

    sym_connected_points = prepro_edge(graph)
    res = list(connected_components(sym_connected_points))
...
def test_plot2d_data():
    ...
    pregraph = knn_graph(df, knn, symmetrical=True, verbose=False)
