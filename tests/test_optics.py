...
def test_scan_neigh1():
    data = {0: np.array([1, 1]), 1: np.array([2, 1]), 2: np.array([1, 4]), 3: np.array([1, 1.5]), 4: np.array([2, 1.5])}
...
def test_ExtractDBSCANclust():
    ...
    assert sorted(ExtractDBSCANclust(res[0], res[1], 1).values()) == [0, 0, 0, 1, 1, 1]

def test_OPTICS():
    D = np.array([[0, 0], [0, 1], [3, 3],
                  [0, 2], [1, 1], [3, 5], [3, 4],
                  [1, 2], [1, 3]])

    expected0 = {'5': np.inf, '6': 1.0, '2': 1.0, '8': 2.0, '7': 1.0, '3': 1.0, '4': 1.0, '1': 1.0, '0': 1.0}
    expected1 = {'5': 1.0, '6': 1.0, '2': 1.0, '8': 1.0, '7': 1.0, '3': 1.0, '4': 1.0, '1': 1.0, '0': 1.0}
